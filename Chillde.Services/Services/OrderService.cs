        ...
        public async Task<ResponseModel> GetOrderDetail(Guid orderId)
        {
            try
            {
                ...
                var model = new OrderDetailModel
                {
                    ...
                    Ward = order?.ToWard ?? string.Empty,
                    District = order?.ToDistrict ?? string.Empty,
                    Province = order?.ToProvince ?? string.Empty,
                    Quantity = order?.Quantity ?? 1,
                    ShipmentCode = order?.ShipmentCode ?? string.Empty,
                    ...
                    CancelOrderReason = order?.CancellationReason?.Name ?? string.Empty,
                    AutoCancelOrderReason = order?.SystemCancelReason?.ToString() ?? string.Empty,
                    ...
                    VoucherUsages = order?.VoucherUsageLogs?.Select(_ => new VoucherUsageModel
                    {
                        ...
                        VoucherCode = _.Voucher?.Code ?? string.Empty,
