...
public class AccountService : IAccountService
{
    ...
    public async Task<ResponseDashboardModel<ArtisanDashboardModel>> GetArtisanDashboard(DashboardFilterModel dashboardFilterModel)
    {
        ...
        int totalOrder = await _unitOfWork.Context.Orders
            .Where(x => x.CreationDate >= startDate && x.CreationDate <= endDate  && x.Package.CreatedById == currentUserId && !x.IsDeleted)
            .CountAsync();

        int totalPendingOrder = await _unitOfWork.Context.Orders
            .Where(x => x.CreationDate >= startDate && x.CreationDate <= endDate && x.Package.CreatedById == currentUserId && x.Status == OrderStatus.Pending && !x.IsDeleted)
            .CountAsync();
