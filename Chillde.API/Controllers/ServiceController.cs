...
namespace Chillde.API.Controllers
{
    ...
    public class ServiceController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetRecommendations([FromQuery] ServiceFilterModel serviceFilterModel)
        {
            try
            {
                var acceptLanguage = Request.Headers["Accept-Language"].ToString();
                var sourceLanguageCode = LanguageHelper.GetSourceLanguageCode(acceptLanguage);
                var targetLanguageCode = LanguageHelper.GetTargetLanguageCode(sourceLanguageCode);
                var result = await _serviceService.GetAllWithSuggestion(serviceFilterModel, sourceLanguageCode, targetLanguageCode);
                return StatusCode(result.Code, result);
            }
            catch (Exception ex)
            {
